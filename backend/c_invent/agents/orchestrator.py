...
Return a top-level JSON object with 'summary', 'facts', 'assumptions', and task-specific sections. Return JSON only."""
    ...
    def _deterministic_discovery(self, pid, prompt, reason):
        ...
        try:
            ...
            docs = self.store.documents(pid)
            analysis = analyze_intake(prompt, docs) or {}
        except Exception:
            analysis = {}

        domains = [d.get("domain") for d in analysis.get("domain_signals", []) if d.get("domain")]
        ...
        return {
            ...
            "objectives": use_cases or ["Confirm business objectives in a discovery workshop"],
            "processes": use_cases or [],
            "actors": [],
            "systems": sources,
            "sources": sources,
            "requirements": requirements,
            "assumptions": [],
            "unknowns": missing or ["Detailed scope pending customer confirmation"],
            "next_steps": [
